---
title: "ChatOps with EmojiChef"
labNumber: 11
duration: "60 minutes"
day: "Day 2, Session 4"
hardwareRequired: false
widgets: ["MarkCompleteButton", "ValidateOutputPaster", "EmojiChef"]
---
...
Try the encoder live below, then verify the known test vectors. The widget uses the same
`EmojiChefQuick` class lifted directly from the Worker source — what you type here is
what the Worker decodes.

<EmojiChef client:load />

> **Note:** The static test-vector table from the original README has been replaced by
> the live widget above. Enter `HSC`, `status`, or `reboot` to verify the canonical
> vectors interactively. The widget also supports the "regenerate" button to call
> `/api/playground/emojichef/encode` against the live Worker.

Verify the vectors yourself using Node if preferred:
...
- Use the EmojiChef widget above to verify your encoding before posting.
