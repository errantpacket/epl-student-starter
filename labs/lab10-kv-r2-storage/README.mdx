...
widgets: ["MarkCompleteButton", "ValidateOutputPaster"]
---
...
> **Self-referential hook:** The downloadable artifacts on this page (build scripts, schemas, enrollment templates) are served from R2 with signed URLs — exactly the pattern you will implement in this lab. The "View source" links in the sidebar call `/api/source/:file` on this site's Worker, which mints a time-limited signed GET URL and streams the file from R2. When your `handleArtifactGet()` is working, it will look almost identical.
