...
- **Local Dev Container.** Install [Docker Desktop][docker-desktop] (or
  [Podman Desktop][podman-desktop]) and the [Dev Containers VS Code
  extension][devc-ext]. Open this repo in VS Code and click "Reopen in
  Container." The pre-pulled OpenWrt rootfs sibling image takes ~15 MB.
- **GitHub Codespace.** Click **"Open in Codespaces"** on the lab page or
  visit [`codespaces.new/errantpacket/epl-student-starter`][cs-link]. The
  free [personal Codespaces tier][cs-pricing] (120 core-hours/month) is more
...
Once the operator console is up, verify the engagement stack:
