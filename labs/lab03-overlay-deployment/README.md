...
```sh
ssh root@192.168.8.1 'cat /etc/openwrt_release | grep REVISION'
...
```sh
scp "$BIN" root@192.168.8.1:/tmp/sysupgrade.bin
ssh root@192.168.8.1 'sha256sum /tmp/sysupgrade.bin'
...
```sh
ssh root@192.168.8.1 'cat /etc/openwrt_release'
...
```sh
ssh root@192.168.8.1 'block detect'
```
...
```sh
ssh root@192.168.8.1 'mkfs.ext4 -L extroot /dev/sda1'
```
...
```sh
ssh root@192.168.8.1 'df -h /overlay'
```
...
```sh
ssh root@192.168.8.1 'mount | grep overlay'
...
```sh
ssh root@192.168.8.1 'opkg update && opkg install tailscale cloudflared python3-light'
```
...
ssh root@192.168.8.1 'python3 --version'
...
```sh
ssh root@192.168.8.1 'block info /dev/sda1'
# UUID should appear

ssh root@192.168.8.1 'uci show fstab'
# Verify fstab.extroot.uuid matches block info output exactly

ssh root@192.168.8.1 'logread | grep -i extroot'
...
```sh
ssh root@192.168.8.1 'opkg update && opkg install e2fsprogs'
...
```sh
ssh root@192.168.8.1 'cat /etc/opkg/distfeeds.conf'
...
scp /tmp/cloudflared-mipsle root@192.168.8.1:/usr/sbin/cloudflared
ssh root@192.168.8.1 'chmod +x /usr/sbin/cloudflared && cloudflared --version'
```
