...
CF Access has two failure shapes for service tokens, and they point at different
root causes:

- **HTTP 302** redirect to `*.cloudflareaccess.com/cdn-cgi/access/login/...` —
  Access does not recognize the credentials at all. The most common cause is that
  the token was minted in **Service Auth > Service Tokens** but never attached
  to the Access Application's policy in step 7. A token at the account level is
  not authorized to reach any application until it is explicitly added to a
  policy's Include list.
- **HTTP 401** with a JSON error body — Access recognized the credentials but
  rejected them. Causes: client ID and client secret swapped (the ID ends in
  `.access`, the secret does not); the token expired; the token was deleted from
  Service Auth after it was attached.

In both cases the fix lives in the dashboard: Access > Applications >
`fleet-gateway-api` > policy `operator-email-allowlist` > Edit, and confirm the
Service Token selector lists your token under Include. If the symptom is 302,
the token is missing from that list. If the symptom is 401, the token is listed
but the credentials in your `output/access-tokens.json` do not match what Service
Auth shows.
