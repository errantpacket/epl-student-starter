...
# Uses jsonfilter (available in the devcontainer) with a fallback to grep.
# ---------------------------------------------------------------------------
if command -v jsonfilter >/dev/null 2>&1; then
    ...
    # Fallback: grep + sed — brittle but works for single-line JSON values
    SERVICE_TOKEN_ID=$(grep -o '"service_token_id":"[^"]*"' "$ACCESS_TOKENS_JSON" \
        | sed 's/"service_token_id":"//;s/"//')
    SERVICE_TOKEN_SECRET=$(grep -o '"service_token_secret":"[^"]*"' "$ACCESS_TOKENS_JSON" \
        | sed 's/"service_token_secret":"//;s/"//')
fi
...
# ---------------------------------------------------------------------------
TMPDIR="/tmp/bake-$$"
OVERLAY_DEFAULTS="${TMPDIR}/etc/uci-defaults"
...
# ---------------------------------------------------------------------------
# Update the build manifest with sealed-image metadata
# ---------------------------------------------------------------------------
MANIFEST="${OUTPUT_DIR}/build-manifest.json"
if [ -f "$MANIFEST" ]; then
    # Append sealed_image fields by rewriting the file (POSIX sed approach)
...
printf '  scp %s root@192.168.8.1:/tmp/sealed.bin\n' "$SEALED_BIN"
printf '  ssh root@192.168.8.1 "sysupgrade -n /tmp/sealed.bin"\n'
